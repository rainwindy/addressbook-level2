...
public class ParserTest {
	...
	public void findCommand_invalidArgs() {
		...
		final String resultMessage = String.format(MESSAGE_INVALID_COMMAND_FORMAT, FindCommand.MESSAGE_USAGE);
		parseAndAssertIncorrectWithMessage(resultMessage, inputs);
	}
	...
	public void findCommand_validArgs_parsedCorrectly() {
		...
		final FindCommand result = parseAndAssertCommandType(input, FindCommand.class);
	...
	private static Person generateTestPerson() {
		try {
			return new Person(new Name(Name.EXAMPLE), new Phone(Phone.EXAMPLE, true), new Email(Email.EXAMPLE, false),
	...
	private static String convertPersonToAddCommandString(ReadOnlyPerson person) {
		String addCommand = "add " + person.getName().fullName + (person.getPhone().isPrivate() ? " pp/" : " p/")
				+ person.getPhone().value + (person.getEmail().isPrivate() ? " pe/" : " e/") + person.getEmail().value
	...
	/**
	 * Asserts that parsing the given inputs will return IncorrectCommand with
	 * the given feedback message.
	 */
	...
	/**
	 * Utility method for parsing input and asserting the class/type of the
	 * returned command object.
	 *
	 * @param input
	 *            to be parsed
